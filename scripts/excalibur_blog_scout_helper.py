...
def main() -> int:
    ...
    if args.suggest_next:
        ...
        # Prefer AS## for Авто-Сейлс pool; fall back to B## for legacy.
        as_nums = [int(m.group(1)) for t in existing if (m := re.match(r"AS(\d+)", t["topic_id"], re.I))]
        b_nums = [int(m.group(1)) for t in existing if (m := re.match(r"B(\d+)", t["topic_id"], re.I))]
        if as_nums or any(t["topic_id"].upper().startswith("AS") for t in existing):
            next_id = f"AS{max(as_nums, default=0) + 1:02d}"
        else:
            next_id = f"B{max(b_nums, default=0) + 1:02d}"
