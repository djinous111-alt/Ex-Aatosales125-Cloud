...
2. Web research 15–25 мин: используй инструмент `**WebSearch`** Курсора для глубинного анализа ТОП-5 конкурентов в реальном времени. GitHub/docs/community нужны для **технических** тем (см. technical_topic gate); для non-tech how-to достаточно отраслевых/официальных источников. Итоговый угол обязан быть beginner-first: что новичку нажать, подключить, проверить и как не сломать процесс. Приоритетный источник фактов — `fact-bank.md`.
...
8. **Technical vs non-tech:** `excalibur_blog_research_notes_gate.py` ставит `technical_topic` только по полям карточки темы (h1/queries/intent/slug) с **token-match** (`ai` ≠ `pain`, `ии` ≠ `Японии`). Автоимпорт из Японии/Кореи/Китая и бытовые логистические чек-листы — **non-tech**: не требуй GitHub ≥3 и official developer docs. Секция `github_evidence` может быть N/A с пояснением. Если официальный портал (ЭПТС и т.п.) отдал HTTP 5xx — бери вторичные отраслевые источники и пометь fallback в notes.
