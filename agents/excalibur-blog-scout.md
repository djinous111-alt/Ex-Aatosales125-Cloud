...
2. **Определение следующего ID:** Запустить `python3 scripts/excalibur_blog_scout_helper.py --suggest-next`. ID = max(pool Bxx, article dirs Bxx-*, ledger, `memory/scout-topic-id-floor.json`, env floor) + 1. AS-only pool ≠ B01, если WP/floor выше. После publish обнови floor watermark.
