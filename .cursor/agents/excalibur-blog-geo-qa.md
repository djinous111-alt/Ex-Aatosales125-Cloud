...
Если typed Task `excalibur-blog-geo-qa` отсутствует в Cloud `subagent_types`, Директор запускает эту роль через `Task(generalPurpose)` с этим файлом + `.cursor/skills/excalibur-geo-qa/SKILL.md`. Parent не выполняет GEO QA сам.
