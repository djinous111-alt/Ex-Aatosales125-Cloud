...
8. **`.gov.ru` 5xx:** если WebFetch/открытие minpromtorg.gov.ru (или другого официального) возвращает HTTP 5xx — сделай 1–2 retry с backoff, затем опирайся на зеркала ФНС (`nalog.gov.ru`), Гарант/Консультант и надёжные СМИ, цитирующие тот же документ/цифру. Канонический `.gov.ru` URL оставляй в таблице источников с пометкой о 5xx; **не** ставь research blocker из‑за одного 500 на первичном файле.
