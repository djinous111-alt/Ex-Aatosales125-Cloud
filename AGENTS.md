...
Dev-окружение (dependency-refresh) настраивается через `.cursor/environment.json` → `.cursor/cloud-agent-install.sh` (ставит `requirements.txt` + `requests`/`python-dotenv`). Отдельного lint/test/build нет: «приложение» — это Python-CLI в `scripts/`. Основные проверки-как-запуск: `python3 scripts/excalibur_blog_today.py`, QA-скрипты (`excalibur_blog_html_linter.py`, `excalibur_blog_slop_detector.py`) на статьях в `memory/blog/articles/`, генерация `excalibur_blog_llms_generator.py --out-dir <dir>`, и локальный просмотр статьи `excalibur_blog_preview.py --article-dir <dir> --serve --port <port>`.

Готча preflight: `scripts/excalibur_blog_doctor.py` завершается с кодом 1 из-за устаревшей проверки `llms generator supports --blog-path` — реальный флаг в `excalibur_blog_llms_generator.py` называется `--blog-dir`. Это давняя нестыковка в самом doctor-скрипте, а не проблема окружения; остальные проверки (Pillow/numpy/interlinker) проходят. WARN про SSH/PUBLIC_SITE_URL — норма без publish-секретов.

Окружение управляется файлом `.cursor/environment.json` (Docker `build` + `install`), а не db-backed snapshot. Не создавай/не сохраняй snapshot-обновление окружения из дашборда: при committed Docker-`build` это конфликтует и даёт «failed to sync environment: 400». Обновление зависимостей меняй только в `requirements.txt` / `.cursor/cloud-agent-install.sh`. Если run стартует с source `Override` (`environmentJsonPath: null`), значит используется сохранённое personal/team окружение поверх репозитория — почини/удали его в Dashboard → Cloud Agents → Environments, чтобы резолв упал обратно на репозиторный `.cursor/environment.json`. Секреты не зависят от build-конфига и инжектятся как env vars.
