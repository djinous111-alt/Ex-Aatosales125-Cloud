...
4. **Валидация спроса (Yandex Wordstat):** 
   - Сначала вызвать `wordstat_get_top_requests` сервера `MCP-KV` (legacy alias: `user-mcp-kv`) для широкого parent-кластера (например, `llms.txt`, `rag система`, `чат боты для бизнеса`), затем для узкого how-to запроса.
