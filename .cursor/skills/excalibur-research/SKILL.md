...
1. **Анализ спроса через Wordstat API:**
  ...
  - Если ответ пустой / только `totalCount` без списка фраз (часто на длинных secondary):
    - **Не блокируй research.** Укороти query (cluster-first: ядро кластера без лишних хвостов) и повтори `wordstat_get_top_requests`.
    - Примеры: `свх авто владивосток стоимость транзита` → `свх авто владивосток`; узкий how-to → широкий parent.
    - В notes явно пометь пустые фразы как незафиксированные и опирайся на успешный короткий запрос.
...
7. GitHub evidence (≥3) обязателен только если topic card — technical (AI/агенты/MCP/API…). Автологистика/растаможка без tech-маркеров в h1/query не требует GitHub; community/docs достаточно.
