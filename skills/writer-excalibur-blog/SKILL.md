...
7. Каждый H2: рекомендация с **imperative-маркерами** из `recommendation_markers_ru` в `memory/brief/editorial-policy.json`. Utility gate считает подстроки буквально: `сделайте`, `не делайте`, `проверьте`, `используйте`, `чеклист`, `шаг `, `workflow` и т.д. Заголовки «Делать / Не делать» и слово «чек-лист» (с дефисом) **не** закрывают `min_recommendation_markers` — пиши `не делайте` / `чеклист` без дефиса в теле.
