...
5. Hook 350–500 символов: боль → ответ → **что сделает читатель**. Явно используй pain/outcome лексику (`проблем`/`ошиб`/… и `результат`/`получите`/`проверьте`/…) — human-voice и utility gates требуют ≥2 pain и ≥3 outcome маркеров.
