...
2. Web research 15–25 мин: используй инструмент `**WebSearch`** Курсора для глубинного анализа ТОП-5 конкурентов в реальном времени. GitHub/docs/community нужны для фактов, но итоговый угол обязан быть beginner-first: что новичку нажать, подключить, проверить и как не сломать процесс. Приоритетный источник фактов — `fact-bank.md`. Gate `technical_topic` (и требование `github_urls >= 3`) срабатывает только на **токены** вроде AI/MCP/API/RAG/агент — не на подстроки в `reader_pain` / русских окончаниях (`фотофиксации`). Для non-tech ниш (автологистика и т.п.) достаточно docs/community evidence без принудительного GitHub.
