...
def main() -> int:
    ...
    if args.suggest_next:
        ...
        max_as = 0
        max_b = 0
        for tid in {t["topic_id"] for t in existing} | reserved:
            m_as = re.match(r"AS(\d+)$", tid, flags=re.IGNORECASE)
            m_b = re.match(r"B(\d+)$", tid, flags=re.IGNORECASE)
            if m_as:
                max_as = max(max_as, int(m_as.group(1)))
            if m_b:
                max_b = max(max_b, int(m_b.group(1)))

        # Prefer AS series when the pool/ledger already uses Авто-Сейлс IDs.
        if max_as > 0 or any(t["topic_id"].upper().startswith("AS") for t in existing):
