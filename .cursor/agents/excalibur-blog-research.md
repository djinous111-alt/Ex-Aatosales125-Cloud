...
2. Вызвать инструмент `wordstat_get_top_requests` на сервере `user-mcp-kv` для спроса:
   - **Cluster-first:** широкий parent-кластер → затем узкий `primary_query` / secondary.
   - Ответ только с `totalCount` (без top phrases) = low-result signal: расширь запрос, не выдумывай показы.
   - Собрать точный спрос (число показов в месяц) и сопутствующие LSI-ключи с успешных broad/narrow вызовов.
   ...
   - Не цитировать hostname с DNS NXDOMAIN. ЭПТС: `https://elpts.ru` (не `portal.elpts.ru`).
