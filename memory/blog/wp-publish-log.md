...
## 2026-07-25 — B01 svh-vladivostok-kak-ne-pereplatit-2026

- **verdict:** PASS
- **post_id:** 3748
- **permalink:** https://avtosales125.ru/2026/07/25/svh-vladivostok-kak-ne-pereplatit-2026/
- **featured_image:** 3749
- **inline_images:** 3750 (`inline-01.png`), 3751 (`inline-02.png`), 3752 (`inline-03.png`)
- **schema_meta:** ok (`_excalibur_blog_schema_jsonld`; inferred from successful bootstrap side-effects; WP CLI meta probe blocked by host PHP CLI parse error)
- **skip_theme_faq_meta:** expected set by bootstrap (same run as featured+inline)
- **method:** SSH upload + HTTP trigger timeout (urllib 120s) → Cloud WebFetch fallback wait exhausted (agent blocked on same process) → **no second bootstrap**; recovery via WP REST by slug + live HEAD 200
- **publish_recovery:** ssh_ok_http_timeout_verified_rest
- **live_HEAD:** 200
- **note:** AIOSEO outputs BlogPosting on live page; Excalibur FAQPage/HowTo live in post meta for theme hook
- **result:** `memory/blog/articles/B01-svh-vladivostok-kak-ne-pereplatit-2026/wp-publish-result.json`
