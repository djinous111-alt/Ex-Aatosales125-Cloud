...
2. **Определение следующего ID:** `python3 scripts/excalibur_blog_scout_helper.py --suggest-next`. Helper берёт high-water B* из `blog-topics.md` + ledger + article dirs + (если есть `PUBLIC_SITE_URL`) live WP slug map. Если ledger неполный vs WP — **не** брать B01 вслепую; синхронизируй `shared/published-articles.md` или задай `EXCALIBUR_B_ID_FLOOR`.
