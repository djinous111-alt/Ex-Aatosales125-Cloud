...
2. **Определение следующего ID:** Запустить скрипт `scripts/excalibur_blog_scout_helper.py --suggest-next` для следующего ID активной серии (`AS*` или `B*`; напр. `AS20`, не ложный `B01`).
