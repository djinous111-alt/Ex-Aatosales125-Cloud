...
7. Каждый H2: рекомендация с литералами utility gate — `Делать:` / `Не делать:` (или «Сделайте» / «Не делайте») и/или `чек-лист` / `чеклист`. Маркеры боли/результата — из `memory/brief/editorial-policy.json` (`pain_markers_ru` / `outcome_markers_ru`).
