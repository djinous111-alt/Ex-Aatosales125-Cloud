...
**Канон:** typed `excalibur-blog-*` (в т.ч. `excalibur-blog-geo-qa`) часто нет в Cloud Task enum → сразу **отдельный `Task(generalPurpose)` на каждую роль** с `.cursor/agents/<role>.md` + skill path. Не ретрай typed name.
