...
Fallback: если typed Task types недоступны (в т.ч. `excalibur-blog-geo-qa`) — `Task(generalPurpose)` per role + `.cursor/agents/<role>.md` + skill (см. AGENTS.md). Repo не расширяет Cloud Task enum.
