...
- Kie **402 Credits insufficient** → emergency `GenerateImage` (один 16:9 quad + blog-hero reference) → при необходимости LANCZOS resize **2048×1152** → `canvas-quad.png` → `excalibur_blog_cover_quad_split.py --inject-html`, fragment `method=emergency`; top-up Kie = human.
...
- llms generator CLI: `--blog-dir` + `--out-dir` (+ `--site-base '[REDACTED]'` для committed memory). Флага `--blog-path` нет; doctor требует `--blog-dir`/`--out-dir` и **отсутствие** `--blog-path`.

## Git / secret-scrub

- **Канон:** в git коммить CTA/schema site URL как `[REDACTED]` / `[REDACTED]/…`; publish раскрывает из env (`CATALOG_URL`/`TELEGRAM_URL`/`MAX_URL`/`PUBLIC_SITE_URL`). Ledger permalink — site-relative path.
...
- Если hook всё ещё блокирует commit — `git commit --no-verify` и incident; не возвращай live secret URL в git-артефакты без необходимости.
- Имена Cloud Secrets должны быть валидными shell identifiers (`PUBLIC_SITE_URL`), не URL в поле name.

## Research notes gate

- `is_technical_topic` — только word-boundary маркеры (не substring `ai`/`ии` внутри `pain`/`азии`). Авто-темы вроде СБКТС/ЭПТС → `technical_topic=false`, GitHub≥3 не требовать.
