...
11a. **CTA href:** бери `CATALOG_URL` / `TELEGRAM_URL` из env в живые `href`. **Не** копируй `[REDACTED]` из conversion-map/site-brief в `href` (битые ссылки). Публичный brand host без trailing slash допустим, если exact env value ловит secret-scan; не пиши секреты в лог.
