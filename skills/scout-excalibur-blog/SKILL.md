...
* Считай `shared/published-articles.md`, `memory/blog/articles/B*-*`, пул `memory/topics/blog-topics.md`.
* Вызови helper (python3):
  ```bash
  python3 scripts/excalibur_blog_scout_helper.py --suggest-next
  ```
  Helper считает high-water B* по topics + ledger + article dirs + live WP (если `PUBLIC_SITE_URL`).
  Если в выводе `WP_NOTE` про untracked slugs / неполный ledger — **не** стартуй с B01; синхронизируй ledger или задай `EXCALIBUR_B_ID_FLOOR`.
  Запомни `Next available topic ID` и Unwritten topic IDs.
