...
Typed Task `excalibur-blog-*` в Cloud **недоступен** (канон) → сразу **отдельный `Task(generalPurpose)` на каждую роль** с `.cursor/agents/<role>.md` + skill path. Не жди enum registration.
