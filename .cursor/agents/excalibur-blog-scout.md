...
6. **Генерация карточки темы:** Сформировать новую карточку строго по канону **utility-only** (режим B, how_to/checklist/comparison) и **дописать (append)** её в конец файла `memory/topics/blog-topics.md`. В `h1` **и** `primary_query` обязателен utility-маркер (`как` / `чек-лист` / `сравнение`…), согласованный с `search_intent`. Перед append: `python3 scripts/excalibur_blog_utility_gate.py --topic-id {ID}` PASS. В `h1`, `h2_outline`, `faq_hints` избегай формулировок "для профи", "архитектура enterprise", "продвинутый стек"; пиши как для человека, который делает первый рабочий шаг.
