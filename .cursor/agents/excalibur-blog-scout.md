...
2. **Определение следующего ID:** Запустить `python3 scripts/excalibur_blog_scout_helper.py --suggest-next` (учитывает ledger + article dirs + live WP + AS*/B*). Не бери B01/B02 без сверки с WP; при необходимости `EXCALIBUR_TOPIC_ID_FLOOR=B0N`.
