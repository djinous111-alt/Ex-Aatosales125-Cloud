...
2. `python3 scripts/excalibur_blog_llms_generator.py --blog-dir memory/blog/articles --site-base ${PUBLIC_SITE_URL} --out-dir memory/blog` (без `--blog-path`)
3. `promotion-checklist.md` из template; перед commit redact `PUBLIC_SITE_URL` → `[REDACTED]` в llms/checklist.
