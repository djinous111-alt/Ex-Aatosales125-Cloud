...
Перед запуском сверяй флаги через `python3 scripts/excalibur_blog_llms_generator.py --help`.
Устаревший `--blog-path` **не существует** в CLI (только `--blog-dir` / `--out-dir` / `--site-base`).
