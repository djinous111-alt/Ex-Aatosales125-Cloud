...
def gate_article(article_dir: Path, policy: dict[str, Any]) -> dict[str, Any]:
    ...
    # Empty marker lists must not force counts=0 BLOCK (INC-20260722-2116).
    # Keep lists in editorial-policy.json in sync with human_voice_gate PAIN/OUTCOME markers.
    if not pain_markers:
        warnings.append(
            "pain_markers_ru отсутствует в editorial-policy.json — проверка боли пропущена"
        )
    else:
    ...
    if not outcome_markers:
        warnings.append(
            "outcome_markers_ru отсутствует в editorial-policy.json — проверка outcome пропущена"
        )
    else:
        min_outcome = int(req.get("min_outcome_markers") or 3)
        if outcome_count < min_outcome:
            errors.append(
                f"слабо раскрыта польза/результат: outcome_markers={outcome_count} < {min_outcome}"
            )
    ...
    return {
        ...
        "metrics": {
            ...
            "pain_markers": pain_count,
            "outcome_markers": outcome_count,
