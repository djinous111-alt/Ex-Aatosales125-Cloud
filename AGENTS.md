...
Если Cloud API не принимает typed Task `excalibur-blog-*` (включая **`excalibur-blog-geo-qa`**, research, writer, cover, schema, indexer, publish, fixer):

- **сразу** отдельный `Task(generalPurpose)` на **каждую** роль — не трать retry на typed-only;
- передай путь `.cursor/agents/<role>.md` и `.cursor/skills/<skill>/SKILL.md` (для GEO QA: `.cursor/agents/excalibur-blog-geo-qa.md` + `.cursor/skills/excalibur-geo-qa/SKILL.md`);
