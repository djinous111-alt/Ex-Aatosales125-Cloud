...
def load_existing_topics(root: Path) -> list[dict[str, str]]:
    """Parse topic cards with ## Bxx or ## ASxx headings from blog-topics.md."""
...
def next_series_id(existing: list[dict[str, str]], prefix: str) -> str:
    max_num = 0
    for t in existing:
        m = re.match(rf"{prefix}(\d+)$", t["topic_id"], flags=re.IGNORECASE)
        if m:
...
def main() -> int:
    ...
    if args.suggest_next:
        ...
        next_b = next_series_id(existing, "B")
        next_as = next_series_id(existing, "AS")
        as_count = sum(1 for t in existing if t["topic_id"].startswith("AS"))
        b_count = sum(1 for t in existing if t["topic_id"].startswith("B"))
        print(f"Next available topic ID (B-series): {next_b}")
        print(f"Next available topic ID (AS-series): {next_as}")
        print(f"Total topics in pool (blog-topics.md): {len(existing)} (AS={as_count}, B={b_count})")
        ...
        print(
            "NOTE: Also reconcile EXCALIBUR_RECENT_WP_POSTS from "
            "`python3 scripts/excalibur_blog_today.py` — live WP slugs may be absent from ledger."
        )
        return 0
    ...
    if args.check_query:
        ...
        if warnings:
            ...
            return 1
        print("✅ NO CANNIBALIZATION RISK: Query is clean and unique vs AS/B pool.")
        print(
            "NOTE: Still compare against EXCALIBUR_RECENT_WP_POSTS from today.py "
            "(live site may have posts not in ledger)."
        )
        return 0
