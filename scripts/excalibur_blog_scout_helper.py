...
from excalibur_topic_ids import (
    CARD_TOPIC_RE,
    suggest_next_topic_id,
    topic_id_from_dirname,
)
...
def load_active_article_topics(root: Path) -> set[str]:
    ...
    for path in articles_dir.iterdir():
        if not path.is_dir():
            continue
        topic_id = topic_id_from_dirname(path.name)
        if topic_id:
            active.add(topic_id)
    return active
...
def load_existing_topics(root: Path) -> list[dict[str, str]]:
    ...
    for match in CARD_TOPIC_RE.finditer(text):
...
def main() -> int:
    ...
    if args.suggest_next:
        ...
        next_id = suggest_next_topic_id(pool_ids, reserved)
