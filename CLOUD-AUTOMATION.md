...
Fallback (**практический default**, пока typed enum пуст): сразу `Task(generalPurpose)` per role + `.cursor/agents/<role>.md` + skill (включая geo-qa). См. AGENTS.md. Не ретраить typed `excalibur-blog-geo-qa`.
