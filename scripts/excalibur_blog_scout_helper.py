...
def main() -> int:
    ...
    if args.suggest_next:
        ...
        # Prefer the dominant prefix already used in the topic pool (AS for Авто-Сейлс).
        prefixes = {"AS": 0, "B": 0}
        for t in existing:
            m = re.match(r"(AS|B)(\d+)", t["topic_id"], flags=re.IGNORECASE)
            if m:
                prefixes[m.group(1).upper()] = max(
                    prefixes[m.group(1).upper()], int(m.group(2))
                )
        if prefixes["AS"] > 0 or any(t["topic_id"].startswith("AS") for t in existing):
            prefix = "AS"
        else:
            prefix = "B"
        next_id = f"{prefix}{prefixes[prefix] + 1:02d}"
