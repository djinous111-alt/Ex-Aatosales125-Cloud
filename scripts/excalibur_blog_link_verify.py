...
def verify_article(
    ...
    *,
    ...
    for href in links:
        if "[REDACTED]" in href or href.strip() == "[REDACTED]":
            results.append(
                {
                    "url": href,
                    "kind": "scrubbed_literal",
                    "status": None,
                    "ok": False,
                    "skipped": False,
                    "method": None,
                    "error": "literal [REDACTED] in href — secret-scrub leak; restore CTA from shared/public-cta.json or conversion-map via python/shell",
                }
            )
            continue
